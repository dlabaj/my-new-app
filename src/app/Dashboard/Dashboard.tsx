...
const Dashboard: React.FunctionComponent = () => {
  const [virtualMachines, setVirtualMachines] = React.useState<Array<{ name: string; provider: string }> | null>(
    null
  );
  ...
  React.useEffect(() => {
    ...
    fetch('/api/virtual-machines/count')
      .then((response) => response.json())
      .then((data: { virtualMachines: Array<{ name: string; provider: string }> }) => {
        if (isMounted) {
          setVirtualMachines(data.virtualMachines);
        }
      })
      .catch(() => {
        if (isMounted) {
            ...
            <Panel>
              <Card isPlain isFullHeight>
                ...
                <CardBody>
                  <DescriptionList>
                    <DescriptionListGroup>
                      <DescriptionListTerm>Total virtual machines</DescriptionListTerm>
                      <DescriptionListDescription>
                        {virtualMachines === null ? (
                          <Skeleton width="3rem" screenreaderText="Loading virtual machine count" />
                        ) : (
                          virtualMachines.length.toLocaleString()
                        )}
                      </DescriptionListDescription>
                    </DescriptionListGroup>
                    {virtualMachines?.map((virtualMachine) => (
                      <DescriptionListGroup key={`${virtualMachine.provider}-${virtualMachine.name}`}>
                        <DescriptionListTerm>{virtualMachine.name}</DescriptionListTerm>
                        <DescriptionListDescription>{virtualMachine.provider}</DescriptionListDescription>
                      </DescriptionListGroup>
                    ))}
                    {virtualMachines?.length === 0 && (
                      <DescriptionListGroup>
                        <DescriptionListTerm>Status</DescriptionListTerm>
                        <DescriptionListDescription>No virtual machines detected on this system</DescriptionListDescription>
                      </DescriptionListGroup>
                    )}
                  </DescriptionList>
                </CardBody>
